...
contract AvalaunchMarketplace is Initializable {
    ...
    function buyPortions(
        ...
        require(
            admin.isAdmin(
                keccak256(abi.encodePacked(owner, sale, portions, prices)).toEthSignedMessageHash().recover(signature)
            ),
        ...
        for(uint i = 0; i < portions.length; i++) {
            uint256 portionId = portions[i];
            require(listedUserPortionsPerSale[owner][sale][portionId] == true, "Portion not buyable.");
